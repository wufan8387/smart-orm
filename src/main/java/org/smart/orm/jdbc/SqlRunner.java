...
public class SqlRunner {
    ...
    private List<Map<String, Object>> convertToMap(ResultSet rs) throws SQLException {
        try {
            ...
            List<Transformer<?>> typeHandlers = new ArrayList<Transformer<?>>();
            ResultSetMetaData rsmd = rs.getMetaData();
            //先计算要哪些列，已经列的类型（Transformer）
            for (int i = 0, n = rsmd.getColumnCount(); i < n; i++) {
                columns.add(rsmd.getColumnLabel(i + 1));
                try {

                    Class<?> type = Resources.classForName(rsmd.getColumnClassName(i + 1));
                    Transformer<?> typeHandler = typeHandlerRegistry.getTypeHandler(type);
                    if (typeHandler == null) {
            ...
            while (rs.next()) {
                ...
                for (int i = 0, n = columns.size(); i < n; i++) {
                    ...
                    Transformer<?> handler = typeHandlers.get(i);
